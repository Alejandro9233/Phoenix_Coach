                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        ZStack(alignment: .leading) {
                            ...
                            DatePicker("", selection: $raceDateVal, displayedComponents: .date)
                                ...
                                .blendMode(.destinationOver)
                                .opacity(0.011)
                                // A date counts only once the athlete picks one. The old
                                // `.onAppear { hasRaceDate = true }` fired the moment this
                                // invisible overlay rendered, so the default `Date()` shipped
                                // as the race date on every unrelated profile save.
